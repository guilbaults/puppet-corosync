...
Puppet::Type.type(:cs_stonith).provide(:pcs, :parent => PuppetX::Voxpupuli::Corosync::Provider::Pcs) do
  ...
  def self.instances
    ...
    raw, status = PuppetX::Voxpupuli::Corosync::Provider::Pcs.run_command_in_cib(cmd)
  ...
  def destroy
    ...
    PuppetX::Voxpupuli::Corosync::Provider::Pcs.run_command_in_cib([command(:pcs), 'stonith', 'delete', @property_hash[:name]])
    @property_hash.clear
  end
  ...
  def flush
    ...
    # try to remoe the default monitor operation (with 'pcs resource')
        if @property_hash[:operations]["monitor"].nil?
          ...
          PuppetX::Voxpupuli::Corosync::Provider::Pcs.run_command_in_cib(cmd, false)
        end
      else
        ...
        if @property_hash[:operations].empty? and not @property_hash[:existing_operations].empty?
          @property_hash[:existing_operations].each do |o|
            ...
            PuppetX::Voxpupuli::Corosync::Provider::Pcs.run_command_in_cib(cmd)
          end
        end
